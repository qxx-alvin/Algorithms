// 代码摘自：https://blog.csdn.net/qq_25379821/article/details/83721379
...
const int maxm = 5;
//int n = maxn, m = maxn;
int Map[maxn][maxm];//map[i][j]=1表示X部的i和Y部的j存在路径,是否可以匹配
int cx[maxn], cy[maxm];
bool vis[maxm];
...
{
	for (int v = 0; v < maxm; v++)//枚举Y部的点，判断X部的u和Y部的v是否存在路径
	{
		//如果存在路径并且还没被标记加入增广路
...
void main()
{
	...
	//}
	Map[0][1] = Map[0][2] = 1;
	Map[1][0] = Map[1][1] = Map[1][3] = Map[1][4] = 1;
	Map[2][1] = Map[2][2] = 1;
	Map[3][1] = Map[3][2] = 1;
	Map[4][3] = Map[4][4] = 1;
